...
export class IDBMirrorVFS extends FacadeVFS {
  ...
  jRead(fileId, pData, iOffset) {
    try {
      ...
      while (pDataOffset < pData.byteLength) {
        ...
        pData.set(
          block.subarray(blockOffset, blockOffset + blockLength),
          pDataOffset,
        )
  ...
  async jLock(fileId, lockType) {
    ...
    switch (lockType) {
      ...
      case VFS.SQLITE_LOCK_RESERVED:
        ...
        console.assert(
          entries[0]?.txId === file.viewTx.txId || !file.viewTx.txId,
        )
        break
  ...
  async #commitTx(file) {
    ...
    txSansData.blocks = new Map(
      Array.from(file.txActive.blocks, ([k]) => [k, null]),
    )
