...
async function getPathComponents(pathname, create) {
  ...
  for (const directory of directories.split("/")) {
    if (directory) {
      directoryHandle = await directoryHandle.getDirectoryHandle(directory, {
        create,
      })
    }
  }
...
export class OPFSAdaptiveVFS extends WebLocksMixin(FacadeVFS) {
  ...
  async jOpen(zName, fileId, flags, pOutFlags) {
    try {
      ...
      const [directoryHandle, filename] = await getPathComponents(
        pathname,
        create,
      )
      file.fileHandle = await directoryHandle.getFileHandle(filename, {
        create,
      })

      if (flags & VFS.SQLITE_OPEN_MAIN_DB && !hasUnsafeAccessHandle) {
        file.handleRequestChannel = new BroadcastChannel(
  ...
  async jAccess(zName, flags, pResOut) {
    try {
      ...
      const fileHandle = await directoryHandle.getFileHandle(dbName, {
        create: false,
      })
  ...
  jRead(fileId, pData, iOffset) {
    try {
      ...
      const bytesRead = file.accessHandle.read(pData.subarray(), {
        at: iOffset,
      })
          ...
          switch (key.toLowerCase()) {
            case "journal_mode":
              if (
                ...
                !["off", "memory", "delete", "wal"].includes(
                  value.toLowerCase(),
                )
              ) {
                throw new Error(
                  'journal_mode must be "off", "memory", "delete", or "wal"',
                )
              }
              break
            case "write_hint":
