...
class File {
  ...
  static async create(pathname, flags) {
    ...
    // @ts-ignore
    file.accessHandle = await handle.createSyncAccessHandle({
...
export class OPFSPermutedVFS extends FacadeVFS {
          ...
          for (const transaction of transactions) {
            ...
            for (const [index, { offset, digest }] of transaction.pages) {
              ...
              if (checksum(data).some((v, i) => v !== digest[i])) {
                throw Object.assign(new Error("checksum error"), {
  ...
  async jClose(fileId) {
    try {
      ...
      if (file?.flags & VFS.SQLITE_OPEN_DELETEONCLOSE) {
        const [directoryHandle, name] = await getPathComponents(
          file.path,
          false,
        )
      ...
      if (file.flags & VFS.SQLITE_OPEN_MAIN_DB) {
        ...
        if (!file.pageSize && iOffset <= 16 && iOffset + bytesRead >= 18) {
          const dataView = new DataView(
            pData.slice(16 - iOffset, 18 - iOffset).buffer,
          )
  ...
  jWrite(fileId, pData, iOffset) {
    try {
      ...
      if (file.flags & VFS.SQLITE_OPEN_MAIN_DB) {
        ...
        file.txActive.fileSize = Math.max(
          file.txActive.fileSize,
          pageIndex * file.pageSize,
        )

        // Track the actual file size.
        file.txRealFileSize = Math.max(
          file.txRealFileSize,
          pageOffset + pData.byteLength,
        )
      } else {
  ...
  async jLock(fileId, lockType) {
    ...
    switch (lockType) {
      ...
      case VFS.SQLITE_LOCK_RESERVED:
        ...
        if (
          !file.locks.hint &&
          !(await this.#lock(file, "hint", POLL_EXCLUSIVE))
        ) {
          return VFS.SQLITE_BUSY
        }
  ...
  async #commitTx(file) {
    ...
    if (file.txActive.oldestTxId) {
      ...
      tx.objectStore("pending").delete(
        IDBKeyRange.upperBound(file.txActive.oldestTxId),
      )
    }
  ...
  async #prepareOverwrite(file) {
    ...
    for (let offset = 0; offset < file.fileSize; offset += file.pageSize) {
      ...
      if (oldOffset < file.fileSize) {
        ...
        if (
          file.accessHandle.read(pageBuffer, { at: oldOffset }) !==
          file.pageSize
        ) {
        ...
        if (
          file.accessHandle.write(pageBuffer, { at: newOffset }) !==
          file.pageSize
        ) {
